...
#this is a draft on "big" ccs config
...
def toggle_saml_dls (x={}):
    """control saml, dls, dlscache flags"""

    d=[x.get('saml',True),x.get('dls',False),x.get('dlscache',False)]

    tc('checkbox presence')
    sx="//*[@name='samlEnforced_boolean']"; se = g.wait.until(EC.element_to_be_clickable((By.XPATH, sx)))
    for c,x in zip(d,['samlEnforced','dlsEnabled','dlsCacheEnabled']):
        sx="//*[@name='"+x+"_boolean']"; se = g.driver.find_element(By.XPATH, sx); s0=se.is_selected()
        if c is not s0:
